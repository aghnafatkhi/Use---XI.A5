...
export default function Kenangan() {
  ...
  return (
    <section id="kenangan" className="relative pt-[60px] pb-[60px] md:pt-[80px] md:pb-[80px] bg-dark-maroon">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-[6vw] lg:px-[8vw] mb-12 md:mb-16 relative text-center md:text-left sr-up">
        ...
        <h2 className="font-instrument italic text-[3rem] md:text-[3.5rem] lg:text-[4rem] text-cream leading-[1.1] mb-6 whitespace-pre-line">
          {"Catatan Perjalanan\nXII.A5."}
        </h2>
        <p className="font-syne text-[0.9rem] md:text-[1rem] text-white/50 max-w-sm mx-auto md:mx-0">
          Dokumentasi momen bersama XII.A5
        </p>
      </div>
      ...
      <div className="relative w-full overflow-hidden sr-fade delay-3">
        ...
        >
           {memories.map((m) => (
              ...
              >
                {/* Polaroid body */}
                <div className="bg-warm-white dark:bg-dark-maroon p-3 pb-10 shadow-[6px_8px_0px_rgba(13,27,75,0.2)] md:shadow-[8px_12px_0px_rgba(0,0,0,0.3)] relative group-hover:shadow-[12px_16px_0px_rgba(0,0,0,0.4)] transition-shadow duration-500">
                  
                  {/* Washi Tape Accent */}
                  <div className="absolute top-[-10px] left-1/2 -translate-x-1/2 w-[60px] h-[20px] bg-gold/35 -rotate-6 z-10 mix-blend-multiply opacity-80"></div>
                  ...
                  <div className="h-[240px] w-full relative flex items-center justify-center overflow-hidden" style={{ background: m.bg?.startsWith('http') || m.bg?.startsWith('data:') ? `url(${m.bg}) center/cover no-repeat` : m.bg }}>
                    <div className="absolute inset-0 bg-dark-maroon/10 group-hover:opacity-0 transition-opacity duration-500"></div>
                    <span className="font-syne font-bold text-[0.8rem] md:text-[0.85rem] text-white/90 uppercase tracking-[0.1em] z-10 px-4 text-center leading-snug drop-shadow-md">
                      {m.title}
                    </span>
                  </div>
                  ...
                  <div className="mt-5 flex flex-col relative px-2">
                     <span className="font-syne-mono text-[0.65rem] text-gold absolute top-[-10px] right-2 tracking-widest font-bold">
                       {m.date}
                     </span>
                     <p className="font-satisfy text-[1.1rem] md:text-[1.2rem] text-dark-maroon dark:text-cream text-center mt-3 leading-relaxed">
                        {m.desc}
                     </p>
                  </div>
                </div>
      ...
      <div className="flex flex-col items-center mt-8 md:mt-12 space-y-6 sr-up delay-4">
         ...
         <div className="flex items-center space-x-16">
            <button 
              onClick={() => scrollBy(-1)}
              className="text-white/50 hover:text-gold transition-colors focus-visible:outline-2 focus-visible:outline-gold p-2 rounded-full hover:bg-white/5"
            ...
            <div className="flex space-x-3">
              {memories.map((_, i) => (
                 <div 
                   key={i} 
                   className={`h-[6px] rounded-full transition-all duration-300 ${activeIndex === i ? 'w-[24px] bg-gold' : 'w-[6px] bg-white/20 hover:bg-white/40'}`}
                 ></div>
              ))}
            </div>
