    ...
    <section id="galeri" className="relative pt-[120px] pb-[120px] bg-[var(--bg-warm)]">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-[6vw] lg:px-[8vw] mb-12 flex flex-col md:flex-row md:items-end justify-between relative">
        ...
        <div className="sr-up md:pr-10">
          <h2 className="font-instrument italic text-[3rem] md:text-[3.5rem] lg:text-[4rem] text-[#5C1414] dark:text-[#F4EDE0] leading-[1.1] whitespace-pre-line mb-6 md:mb-0">
            {"Dokumentasi\nAktivitas Kelas."}
          </h2>
        </div>
          ...
          {filteredItems.map((item, index) => {
            ...
            return (
              ...
              >
                ...
                <div className="absolute inset-x-0 bottom-0 top-1/2 bg-gradient-to-t from-[#3A0A0A]/80 to-transparent translate-y-full flex items-end justify-center pb-6 transition-transform duration-300 ease-out group-hover:translate-y-0 z-20 pointer-events-none">
                  <span className="font-syne font-bold text-[0.8rem] text-[#F4EDE0] tracking-wider uppercase">Buka Foto <span className="ml-1">↗</span></span>
                </div>
              </div>
            );
          })}
      ...
      {/* LIGHTBOX */}
      {lightboxIndex !== null && (
        <div className="fixed inset-0 z-[2000] bg-[rgba(26,8,8,0.98)] flex flex-col items-center justify-center p-4 md:p-8" onClick={() => setLightboxIndex(null)}>
          <button 
            className="absolute top-6 right-6 text-[#F4EDE0]/60 hover:text-[#C4973A] font-syne text-[40px] leading-none focus-visible:outline-2 focus-visible:outline-[#C4973A] transition-colors p-2 z-50"
          ...
          <div className="mt-8 text-center max-w-xl" onClick={(e) => e.stopPropagation()}>
            <h4 className="font-syne font-bold text-[#F4EDE0] text-xl md:text-2xl mb-2">{filteredItems[lightboxIndex].title}</h4>
            <p className="font-syne-mono text-[#C4973A] text-xs uppercase tracking-widest">{filteredItems[lightboxIndex].category.replace('-', ' ')}</p>
          </div>
        </div>
      )}
    </section>
